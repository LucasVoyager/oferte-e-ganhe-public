document.addEventListener('DOMContentLoaded', () => {
    ...
    async function atualizarTalao(event) {
        ...
        if (temPermissao('ADMIN')) {
            try {
                const resposta = await fetch(`http://localhost:3000/api/recebimentoTaloes/atualizarRecebimento/${id_talao}`, {
                    method: 'PATCH',
                    headers: {
                ...
                if (dados) {
                    ...
                    mensagemDiv.textContent = `Erro ao informar recebimento do talao ${dados.error}`
                    mensagemDiv.className = 'mensagem erro'
                    mensagemDiv.style.display = 'block'
                }
                
            } catch (err) {
                console.error('erro ao informar', err)
                mensagemDiv.textContent = `Erro ao informar recebimento do ${err}`
                mensagemDiv.className = 'mensagem erro'
                mensagemDiv.style.display = 'block'
            }
            ...
            try {
                ...
                mensagemDiv.textContent = `Erro ao informar recebimento do talao ${err}`
                mensagemDiv.className = 'mensagem erro'
                mensagemDiv.style.display = 'block'
            }
        }
